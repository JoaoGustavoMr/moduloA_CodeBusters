...
$stmt = $conexao->prepare("SELECT nome, email, senha, tipo FROM usuarios WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $update_stmt = $conexao->prepare("UPDATE usuarios SET nome = ? WHERE id = ?");
    $update_stmt->bind_param("si", $nome, $id_usuario);
    $update_stmt->execute();
    ...
    if (!empty($senha_atual) && !empty($nova_senha)) {
        if (password_verify($senha_atual, $usuario["senha"])) {
            ...
            $update_senha = $conexao->prepare("UPDATE usuarios SET senha = ? WHERE id = ?");
    ...
    <main>
        ...
        <section class="container-form">
            <form method="POST">
                ...
                <input type="text" id="tipo" value="<?php echo htmlspecialchars($usuario['tipo']); ?>" readonly>
